...
local N = (Redis:get(Saidi.."Name:Bot") or "شغف")
...
NamesBot = (Redis:get(Saidi.."Name:Bot") or "شغف")
...
local NamesBot = (Redis:get(Saidi.."Name:Bot") or "شغف")
local BotName = {
'اسمي '..NamesBot..' يروحي',
'اؤمر انا '..NamesBot..'️',
'اسمي '..NamesBot..' يقلبي',
...
if text == (Redis:get(Saidi.."Name:Bot") or "شغف") then
local photo = LuaTele.getUserProfilePhotos(Saidi)
local Jabwa = LuaTele.getUser(Saidi)
local sudo_info = LuaTele.getUser(Sudo_Id)
local sudo_name = sudo_info.first_name
...
local NamesBot = (Redis:get(Saidi.."Saidi:Name:Bot") or "شغف")
local BotName = {
'نعم يروحي',
...
if text == (Redis:get(Saidi.."Name:Bot") or 'شغف').." غادر" or text == 'غادر' or text == 'بوت غادر' then
if not msg.Developers then
return LuaTele.sendText(msg_chat_id,msg_id,'\n*᥀︙ هاذا الامر يخص { '..Controller_Num(3)..' }* ',"md",true)
end
...
local Mostafa = (Redis:get(Saidi.."Name:Bot") or "شغف")
local Jabwa = LuaTele.getUser(Saidi)
local bain = LuaTele.getUser(msg.sender.user_id)
if ChannelJoin(msg) == false then
local reply_markup = LuaTele.replyMarkup{type = 'inline',data = {{{text = 'اضغط للاشتراك', url = 't.me/'..Redis:get(Saidi..'Channel:Join')}, },}}
...
local CmdStart = '\n*᥀︙ مرحبا بك انا بوت '..(Redis:get(Saidi.."Saidi:Name:Bot") or "شغف")..
